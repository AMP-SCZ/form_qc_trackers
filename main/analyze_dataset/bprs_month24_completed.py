...
class BPRSMonth24Completed():
    """
    Standalone analysis script.

    Writes a CSV of every participant whose BPRS form at the month 24
    timepoint is marked complete AND whose completion happened more than
    DAYS_THRESHOLD (60) days before today.
    ...
    (chrbprs_interview_date) — the canonical date the pipeline uses for a
    ...
    OUTPUT_FILENAME = 'bprs_month24_completed_over_60_days_ago.csv'
    ...
    def __init__(self):
        ...
        self.completion_var = form_vars['completion_var']       # bprs_complete
        self.date_var = form_vars[self.DATE_VAR_KEY]            # chrbprs_interview_date
    ...
    def collect_completed_participants(self):
        ...
        for network in ['PRONET', 'PRESCIENT']:
            ...
            try:
                ...
                print(f"[bprs_month24] combined CSV not found for {network}: "
            ...
            if missing_cols:
                print(f"[bprs_month24] {network} month24 CSV missing "
    ...
    def write_output(self):
        ...
        print(f"[bprs_month24] wrote {len(df)} participant(s) "
              f"(BPRS complete at month24, >{self.DAYS_THRESHOLD} days ago) "
              f"to {out_path}")


if __name__ == '__main__':
    BPRSMonth24Completed().run_script()
