...
def _draw_radar(ax, variable_rows: pd.DataFrame, title: str, *,
    ...
    # ``display_site_id`` is a spoke-label override (e.g. "Site 1", "Site 2")
    # supplied by the caller. Only the printed label changes: site_id stays the
    # real identifier everywhere else, including the manifest.
    label_column = ("display_site_id"
                    if "display_site_id" in variable_rows.columns
                    else "site_id")
    site_ids = variable_rows[label_column].map(_text).tolist()
...
def _build_figure(group: pd.DataFrame, network: str, variable: str,
                  ...
                  absent_legend: str = ("Site with no recorded evidence "
                                        "(labelled, no marker)"),
                  spoke_intro: str = "Spokes = every site in",
    ...
    fig.text(
        0.5, 0.040,
        f"{spoke_intro} {scope_label} ({coverage}); {radius_note}.",
...
def _render_charts(scores: pd.DataFrame, site_universe: dict[str, list[str]],
    ...
    # Not a _build_figure keyword: it is consumed here, where the spoke order
    # for the whole variable is already settled, so the numbering runs 1..N
    # across a paginated variable instead of restarting on every page.
    numbered_site_labels = bool(
        figure_options.pop("numbered_site_labels", False))
    frame = scores.copy()
    ...
    try:
        ...
        try:
            ...
            for (network, variable), group in grouped:
                ...
                if numbered_site_labels:
                    ordered_sites["display_site_id"] = [
                        f"Site {position}"
                        for position in range(1, total_sites + 1)]
                ...
                for page_index, page_size in enumerate(page_sizes):
                    ...
                    try:
                        # Save the vector page first. Matplotlib's tight PNG
                        # bounds pass can alter the live figure renderer; saving
                        # PDF afterward may then clip figure-level legends and
                        # footers on dense pages.
    ...
    if roster_all_timepoints and timepoints:
        ...
        if extra:
            ...
            warnings.append(
                ...
                f"charted timepoint(s) ({', '.join(other)}) so later-enrolling "
                "sites remain in the available roster")
    ...
    *,
    ...
    for scope_key, scope_label, scope_networks in _scope_specs(
        ...
        for (timepoint, variable), part in scope_stats.groupby(
            ...
            reference_basis = (
                f"cross-site median of the site {stat_label} over sites with "
                f"n >= {reference_min_n}")
            if len(reference_pool) < _MIN_REFERENCE_SITES:
                ...
                reference_basis = (
                    f"fallback cross-site median of the site {stat_label} "
                    f"over all {len(fallback)} plotted site(s)")
            ...
            reference_pool_sites = set(reference_pool.index)
            ...
            for site in roster_sites:
                ...
                rows.append({
                    ...
                    "reference_pool_member": site in reference_pool_sites,
...
def _validate_site_selection_options(
        site_limit: int, normal_site_count: int) -> None:
    if (isinstance(site_limit, (bool, np.bool_))
            or not isinstance(site_limit, Integral)
            or site_limit < 1):
        raise RadarInputError("site_limit must be a positive integer")
    if (isinstance(normal_site_count, (bool, np.bool_))
            or not isinstance(normal_site_count, Integral)
            or normal_site_count < 0):
        raise RadarInputError(
            "normal_site_count must be a nonnegative integer")


def select_representative_site_scores(
    scores: pd.DataFrame,
    *,
    site_limit: int,
    normal_site_count: int = 2,
) -> pd.DataFrame:
    """Mark a compact, deterministic site subset for each radar chart.

    The input scores already contain references, deviations, and full-precision
    ranks computed from the complete eligible population. Selection therefore
    changes only which spokes are displayed; it cannot move the cross-site
    reference. The selected subset contains a small near-reference anchor set
    drawn from the actual reference pool, then the largest remaining absolute
    deviations.

    Sites without a plotted statistic remain in the returned audit frame but do
    not consume a chart slot. Ties prefer the better-supported site, then site
    id, so repeated runs select exactly the same roster.
    """
    _validate_site_selection_options(site_limit, normal_site_count)

    required = {
        "chart_group", "variable", "site_id", "recorded_in_input",
        "raw_deviation", "site_n", "site_rank_by_deviation",
        "reference_pool_member",
    }
    missing = sorted(required.difference(scores.columns))
    if missing:
        raise RadarInputError(
            "Cannot select representative sites; score table is missing "
            + ", ".join(missing))

    marked = scores.copy()
    marked["selected_for_chart"] = False
    marked["site_selection_role"] = "not_selected"

    for _, group in marked.groupby(
            ["chart_group", "variable"], sort=False, dropna=False):
        deviations = pd.to_numeric(
            group["raw_deviation"], errors="coerce").abs()
        recorded = group["recorded_in_input"].fillna(False).astype(bool)
        candidates = group.loc[recorded & deviations.notna()].copy()
        if candidates.empty:
            continue

        candidates["_deviation_rank"] = pd.to_numeric(
            candidates["site_rank_by_deviation"], errors="coerce")
        candidates["_site_n"] = pd.to_numeric(
            candidates["site_n"], errors="coerce").fillna(-np.inf)
        candidates["_site_key"] = candidates["site_id"].map(
            lambda value: (_text(value).casefold(), _text(value)))

        target_count = min(len(candidates), int(site_limit))

        reference_candidates = candidates.loc[
            candidates["reference_pool_member"].fillna(False).astype(bool)]
        anchor_count = min(
            normal_site_count, max(0, target_count - 1),
            len(reference_candidates))
        near_reference = reference_candidates.sort_values(
            ["_deviation_rank", "_site_n", "_site_key"],
            ascending=[False, False, True], kind="stable").head(anchor_count)
        remaining = candidates.drop(index=near_reference.index)
        largest_count = target_count - len(near_reference)
        largest = remaining.sort_values(
            ["_deviation_rank", "_site_n", "_site_key"],
            ascending=[True, False, True], kind="stable").head(largest_count)

        marked.loc[near_reference.index, "selected_for_chart"] = True
        marked.loc[near_reference.index,
                   "site_selection_role"] = "near_reference"
        marked.loc[largest.index, "selected_for_chart"] = True
        marked.loc[largest.index,
                   "site_selection_role"] = "largest_deviation"
...
def generate_radar_charts_from_combined(
    ...
    *,
    ...
    """Chart site statistics for variables, measured from combined CSVs.

    By default, every study site gets a spoke. site_limit opts one variable into
    a compact display: references and deviations are still calculated from the
    complete eligible pool, then each chart keeps up to normal_site_count sites
    nearest the reference and fills the remaining slots, up to site_limit, with
    the largest absolute deviations. Omitted sites stay in the audit.
    """
    ...
    if site_limit is not None:
        if len(variables) != 1:
            raise RadarInputError(
                "site_limit supports exactly one variable per run")
        _validate_site_selection_options(site_limit, normal_site_count)
    if max_sites_per_chart is not None and max_sites_per_chart < 1:
        raise RadarInputError(
            "max_sites_per_chart must be at least 1, or None to draw every "
    ...
    available_site_universe = {
        group: list(sites) for group, sites in site_universe.items()}

    if site_limit is None:
        scores["selected_for_chart"] = True
        scores["site_selection_role"] = "all_sites"
        chart_scores = scores
        render_site_universe = site_universe
    else:
        scores = select_representative_site_scores(
            scores, site_limit=site_limit,
            normal_site_count=normal_site_count)
        chart_scores = scores.loc[scores["selected_for_chart"]].copy()
        render_site_universe = {}

    selected_sites_by_group = {
        _text(group): sorted(
            {_text(value) for value in part["site_id"]},
            key=lambda value: (value.casefold(), value))
        for group, part in chart_scores.groupby("chart_group", sort=False)
    }
    ...
    selected_variables = (
        ...
        .reset_index(drop=True))
    available_counts = (
        scores.groupby(["chart_group", "variable"], sort=False)["site_id"]
        .nunique().rename("available_site_count").reset_index())
    charted_counts = (
        chart_scores.groupby(["chart_group", "variable"], sort=False)["site_id"]
        .nunique().rename("spoke_site_count").reset_index())
    selected_variables = selected_variables.merge(
        available_counts, on=["chart_group", "variable"], how="left",
        validate="one_to_one")
    selected_variables = selected_variables.merge(
        charted_counts, on=["chart_group", "variable"], how="left",
        validate="one_to_one")
    selected_variables["spoke_site_count"] = (
        selected_variables["spoke_site_count"].fillna(0).astype(int))
    _write_csv_atomic(selected_variables, output / "selected_variables.csv")
    ...
    if site_limit is None:
        selection_description = "all available sites"
        spoke_explanation = (
            "This output does NOT come from a thresholded anomaly report. "
            "Every site statistic is computed directly from the combined CSVs, "
            "and every site in the scanned roster gets a spoke. Sites without "
            "usable data appear as labelled, unmarked spokes. Nothing is "
            "imputed and no absent site is plotted at zero.")
        score_audit_description = (
            "site_deviation_scores.csv has one row per plotted spoke, recorded "
            "or not, with the reference, deviation, and robust z.")
    else:
        selection_description = (
            f"up to {site_limit} measured sites per chart; "
            f"up to {normal_site_count} actual reference-pool site(s) nearest "
            "the reference, then the largest remaining absolute deviations")
        figure_options.update({
            "spoke_intro": "Spokes = selected sites in",
            "footer_note": (
                "Reference uses the complete eligible pool. Display: up to "
                f"{site_limit} measured sites: up to {normal_site_count} "
                "nearest reference-pool site(s), then largest deviations. "
                "Near-reference is descriptive, not clinical."),
        })
        spoke_explanation = (
            "This output does NOT come from a thresholded anomaly report. "
            "Every site statistic, deviation, and full-precision rank is "
            "computed before display selection; the reference uses every site "
            "in the complete eligible reference pool. The chart then shows a "
            "compact representative subset: a small anchor set drawn from that "
            "pool plus the largest remaining absolute deviations. "
            "Near-reference is descriptive of this chart statistic, not a "
            "clinical classification.")
        score_audit_description = (
            "site_deviation_scores.csv retains every available site and marks "
            "selected_for_chart plus site_selection_role; excluded sites "
            "therefore remain auditable.")

    chart_count, pdf_path = _render_charts(
        chart_scores, render_site_universe, output, warnings,
        max_sites_per_chart=max_sites_per_chart,
        ...
        extra_manifest_columns=("chart_group", "chart_scope_label", "scope",
                                ...
                                "reference_pool_member", "selected_for_chart",
                                "site_selection_role"))

    roster_text = "; ".join(
        f"{group}: {len(sites)}"
        for group, sites in sorted(selected_sites_by_group.items())) or "none"
    available_roster_text = "; ".join(
        f"{group}: {len(sites)}"
        for group, sites in sorted(available_site_universe.items())) or "none"
    ...
    if max_sites_per_chart is None:
        spoke_limit_text = (
            "no pagination limit (all selected sites on one page)"
            if site_limit is not None
            else "no limit (every site on one page)")
    else:
        spoke_limit_text = str(max_sites_per_chart)
    readme = textwrap.dedent(f"""\
        ...
        Site selection: {selection_description}
        Sites available per page group: {available_roster_text}
        ...
        the median over every measurable site and the fallback is listed in the
        ...
        n_observed. {score_audit_description}
    ...
    recorded = chart_scores.loc[
        chart_scores["recorded_in_input"].astype(bool)]
    available_recorded = scores.loc[scores["recorded_in_input"].astype(bool)]
    return {
        "output_dir": output,
        ...
        "site_count": int(recorded[["chart_group", "site_id"]]
                          .drop_duplicates().shape[0]),
        "spoke_site_count": int(sum(len(sites)
                                    for sites in selected_sites_by_group.values())),
        "sites_by_network": {group: list(sites)
                             for group, sites in selected_sites_by_group.items()},
        "available_site_count": int(
            available_recorded[["chart_group", "site_id"]]
            .drop_duplicates().shape[0]),
        "available_spoke_site_count": int(sum(
            len(sites) for sites in available_site_universe.values())),
        "available_sites_by_network": available_site_universe,
