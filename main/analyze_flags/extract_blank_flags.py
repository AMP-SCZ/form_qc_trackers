...
class BlankFlagExtractor:
    # Exact blank-value message contract. Substring matching is unsafe because
    # malformed free-text values can themselves contain "Variable is blank."
    BLANK_MESSAGE = "variable is blank"
    ...
    def _collect_blank_records(self, df, subject_col, tp_col, flags_col,
        ...
        for row in df[[subject_col, tp_col, flags_col]].itertuples(index=False, name=None):
            ...
            for entry in split_specific_flag_entries(flags):
                ...
                if normalized_message == self.BLANK_MESSAGE:
                    records.add((
                        network_name,
                        subject_s,
                        variable,
