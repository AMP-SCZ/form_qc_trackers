...
def _from_family_pattern(variable: str, message: str) -> str | None:
    ...
    # Direction A of the missingness reconciliation is emitted only for the
    # per-form missing button that is set to 1.  Trigger variables involved in
    # the same flag do not use the ``_missing`` naming contract and fail closed.
    if "_missing" in variable:
        match = re.fullmatch(
            r".+ marked as missing on the form, but the missing_data form has "
            r"no missing-data triggers set for this timepoint\.",
            message,
            re.IGNORECASE,
        )
        if match:
