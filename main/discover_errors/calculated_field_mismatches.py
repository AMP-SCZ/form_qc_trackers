...
The default ``multipass`` comparison starts from the immutable imported
snapshot, evaluates every eligible target simultaneously, and repeats those
project-wide passes until same- and cross-event calculated references reach a
fixed point. Cycles or evaluation failures are reported rather than allowing a
partial, iteration-order-dependent result. ``direct`` mode remains available
for one-pass imported-snapshot evaluation, and legacy ``propagated`` mode keeps
its same-event ordered-update behavior. Direct-mode downstream mismatches and
calculations whose branching eligibility cannot be safely established are
retained but marked ``semantic_uncertain``. The selected mode is recorded in
diagnostics.
"""
...
CROSS_EVENT_MODES = {
    "direct": "stored_snapshot",
    "propagated": "stored_snapshot",
    "multipass": "iterative_fixpoint",
}
...
class _PropagationPlan:
    """One eligible calculated target participating in fixpoint passes."""

    row_index: int
    row: LoadedRow
    variable: str
    entry: Mapping[str, Any]
    structural_blanks: tuple[str, ...]
    branching_eligibility: str
    branching_reason: str
    extra_semantic_risks: tuple[str, ...]
...
def _is_exact_input_blank(value: Any) -> bool:
    """Treat only true nulls and exact empty text as absent input cells."""
    if value is None:
        return True
    if isinstance(value, str):
        return value == ""
...
def _calculation_input_values(values: Mapping[str, Any]) -> dict[str, Any]:
    """Return a row mapping with nulls and study missing codes blanked.

    Blank keys remain omitted, matching the scanner's established in-memory
    representation; every missing key is presented as ``''`` by redcap_value.
    Reapplying this at row iteration also protects programmatic callers that
    construct LoadedRow objects without going through ``load_rows``.
    """
    normalized_values = {}
    for key, value in values.items():
        normalized = normalize_calculation_input(value)
        if not _is_exact_input_blank(normalized):
...
def _comparison_value(value: Any) -> tuple[str, Any]:
    """Normalize one report value without changing evaluator input rules."""
    kind, normalized = _normalized_value(value)
    if (
        kind == "text"
        and normalized.strip().casefold() == "nan"
    ):
        return "blank", ""
...
def values_match(
    ...
    scalars, whitespace-only cells, and trimmed case-insensitive literal
    ``"NaN"`` values form one comparison-only blank class. Evaluator inputs
    ...
    left_kind, left_value = _comparison_value(left)
    right_kind, right_value = _comparison_value(right)
...
def _merge_duplicate_identity_rows(
    ...
    for key in sorted(all_value_keys):
        ...
        if _is_exact_input_blank(selected):
            merged_values.pop(key, None)
        else:
...
def load_rows(
    ...
    for spec in specs:
            ...
            for chunk in chunks:
                ...
                for values_tuple in chunk.itertuples(index=False, name=None):
                    ...
                    for column, value in zip(columns, values_tuple):
                        # Identity/event/repeat metadata is structural, not a
                        # questionnaire response. A valid subject or repeat
                        # instance literally named ``999`` must not disappear.
                        if column in IDENTITY_COLUMNS:
                            normalized = value
                        else:
                            normalized = normalize_calculation_input(value)
                        if (column not in IDENTITY_COLUMNS
                                and is_calculation_missing_code(value)):
                            missing_code_cell_count += 1
                        # Ordinary blanks have always been omitted so duplicate
                        # rows can merge complementary values. Treat coded
                        # missing values identically while available_columns
                        # continues to preserve structural schema presence.
                        if not _is_exact_input_blank(normalized):
        ...
        input_details.append({
            ...
            "missing_code_cells_replaced_with_blank": missing_code_cell_count,
...
def _target_form_is_definitively_inactive(
    ...
    if missing_variable:
        kind, value = _normalized_value(normalize_calculation_input(
            row.values.get(missing_variable, "")))
    ...
    if (
        ...
        and _is_real_missing(normalize_calculation_input(
            row.values.get(completion_field, "")))
    ):
...
def _result_row(
    ...
    stored = normalize_calculation_input(row.values.get(variable, ""))
    semantic_risk = (
        _semantic_risk(
    ...
    return {
        ...
        "cross_event_mode": CROSS_EVENT_MODES.get(
...
def _output_suppression_reason(result: Mapping[str, Any]) -> str:
    """Return the output-only suppression reason, if any."""
    if result.get("branching_eligibility") != "visible":
        return "branching_not_visible"
    stored_kind, _ = _comparison_value(result.get("stored_value", ""))
    recalculated_kind, _ = _comparison_value(
        result.get("recalculated_value", ""))
    if stored_kind == "blank" and recalculated_kind == "blank":
        return "both_values_blank"
    return ""


def _emit_result_rows(
        results: Sequence[Mapping[str, Any]], *,
        artifacts: ScanArtifacts,
        mismatch_sink: Callable[[Mapping[str, Any]], None] | None) -> None:
    """Emit only visible rows with at least one comparison-nonblank value."""
    for result in results:
        suppression_reason = _output_suppression_reason(result)
        if suppression_reason:
            artifacts.counters[
                f"output_suppressed__{suppression_reason}"] += 1
        elif mismatch_sink is None:
            artifacts.mismatches.append(dict(result))
        else:
            mismatch_sink(result)


def _iteration_raw_states(
        rows: Sequence[LoadedRow]) -> dict[int, dict[str, Any]]:
    """Return normalized mutable copies of pass-zero imported row values."""
    return {
        index: _calculation_input_values(row.values)
        for index, row in enumerate(rows)
    }


def _iteration_row_lookups(
        rows: Sequence[LoadedRow]) -> tuple[
            dict[tuple[str, str, str], int],
            dict[tuple[str, str, str, str, str], int]]:
    nonrepeat: dict[tuple[str, str, str], int] = {}
    repeating: dict[tuple[str, str, str, str, str], int] = {}
    for index, row in enumerate(rows):
        if row.is_repeating:
            repeating[
                (row.network, row.subjectid, row.event_name,
                 row.repeat_instrument, row.repeat_instance)
            ] = index
        elif row.event_name:
            nonrepeat[(row.network, row.subjectid, row.event_name)] = index
    return nonrepeat, repeating


def _iteration_current_values(
        row_index: int, rows: Sequence[LoadedRow],
        raw_states: Mapping[int, Mapping[str, Any]],
        nonrepeat_rows: Mapping[tuple[str, str, str], int],
        fields_by_form: Mapping[str, set[str]]) -> Mapping[str, Any]:
    """Build a current-row view, overlaying repeat data on current base data."""
    row = rows[row_index]
    if not row.is_repeating:
        return raw_states[row_index]
    current: dict[str, Any] = {}
    base_index = nonrepeat_rows.get(
        (row.network, row.subjectid, row.event_name))
    if base_index is not None:
        current.update(raw_states[base_index])
    current.update({
        field_name: ""
        for field_name in fields_by_form.get(row.repeat_instrument, set())
    })
    current.update(raw_states[row_index])
    return current
...
def _build_iteration_snapshot(
        ...
        raw_states: Mapping[int, Mapping[str, Any]],
            ...
            dict[tuple[str, str], dict[Any, Any]],
            dict[tuple[str, str, str], dict[Any, Any]]]:
    """Materialize one immutable-pass view for explicit-event references."""
    by_subject: dict[tuple[str, str], dict[Any, Any]] = defaultdict(dict)
    current_instances: dict[
        tuple[str, str, str], dict[Any, Any]] = defaultdict(dict)
    for index, row in enumerate(rows):
        subject_key = (row.network, row.subjectid)
        values = raw_states[index]
        if not row.is_repeating:
            if row.event_name:
                by_subject[subject_key][row.event_name] = values
            continue
        for instance in _instance_keys(row.repeat_instance):
            for variable, value in values.items():
                ...
                current_instances[
                    (row.network, row.subjectid, row.event_name)][
                        (instance, variable)] = value
    return dict(by_subject), dict(current_instances)


def _resolve_calculated_dependency_cell(
        plan: _PropagationPlan, reference: Mapping[str, Any],
        nonrepeat_rows: Mapping[tuple[str, str, str], int],
        repeating_rows: Mapping[tuple[str, str, str, str, str], int],
        ) -> tuple[int, str] | None:
    """Resolve a calculated reference to the target cell that supplies it."""
    row = plan.row
    variable = str(reference.get("variable", ""))
    source_form = str(reference.get("source_form", "")).strip()
    event = reference.get("event")
    instance = reference.get("instance")
    effective_event = str(event) if event is not None else row.event_name

    if instance is not None:
        instance_key = str(int(instance))
        if source_form:
            source_index = repeating_rows.get((
                row.network, row.subjectid, effective_event,
                source_form, instance_key))
            return ((source_index, variable)
                    if source_index is not None else None)
        candidates = [
            index
            for (network, subjectid, target_event, _form,
                 target_instance), index in repeating_rows.items()
            if (
                network == row.network
                and subjectid == row.subjectid
                and target_event == effective_event
                and target_instance == instance_key
            )
        ]
        return ((candidates[0], variable) if len(candidates) == 1 else None)

    if event is not None:
        source_index = nonrepeat_rows.get(
            (row.network, row.subjectid, effective_event))
        return ((source_index, variable)
                if source_index is not None else None)
    ...
    if (
        row.is_repeating
        and source_form
        and source_form != row.repeat_instrument
    ):
        source_index = nonrepeat_rows.get(
            (row.network, row.subjectid, row.event_name))
        return ((source_index, variable)
                if source_index is not None else None)
    return plan.row_index, variable


def _multipass_state_key(value: Any) -> str:
    """Preserve post-policy representation when deciding fixpoint equality."""
    normalized = normalize_calculation_input(value)
    if _is_exact_input_blank(normalized):
        return ""
    return str(normalized)


def _multipass_unstable_closure(
        unstable: set[tuple[int, str]],
        dependencies: Mapping[
            tuple[int, str], tuple[tuple[int, str], ...]],
        ) -> set[tuple[int, str]]:
    reverse: dict[tuple[int, str], set[tuple[int, str]]] = defaultdict(set)
    for dependent, sources in dependencies.items():
        for source in sources:
            reverse[source].add(dependent)
    queue = list(unstable)
    while queue:
        source = queue.pop()
        for dependent in reverse.get(source, ()):
            if dependent not in unstable:
                unstable.add(dependent)
                queue.append(dependent)
    return unstable


def _scan_multipass_calculated_fields(
        loaded: LoadedInputs,
        plans: Sequence[_PropagationPlan],
        calculations: Mapping[str, Mapping[str, Any]],
        fields_by_form: Mapping[str, set[str]],
        permanently_unavailable: Mapping[tuple[int, str], str], *,
        artifacts: ScanArtifacts,
        inactive_cells: set[tuple[int, str]],
        numeric_tolerance: Decimal,
        blank_result_as_zero: bool,
        mismatch_sink: Callable[[Mapping[str, Any]], None] | None,
        ) -> None:
    """Run synchronous project-wide passes and emit only final outcomes."""
    if not plans:
        artifacts.counters["multipass_passes"] = 0
        artifacts.counters["multipass_converged"] = 1
        return

    rows = loaded.rows
    raw_states = _iteration_raw_states(rows)
    nonrepeat_rows, repeating_rows = _iteration_row_lookups(rows)
    for row_index, variable in inactive_cells:
        raw_states[row_index][variable] = ""
    plan_cells = {(plan.row_index, plan.variable) for plan in plans}
    converted_variables = {
    ...
    dependencies: dict[
        tuple[int, str], tuple[tuple[int, str], ...]] = {}
    unresolved: dict[tuple[int, str], tuple[str, ...]] = {}
    for plan in plans:
        cell = (plan.row_index, plan.variable)
        resolved: list[tuple[int, str]] = []
        missing: list[str] = []
        for reference in plan.entry.get("references", ()):
            if reference.get("kind") != "calc":
                continue
            variable = str(reference.get("variable", ""))
            display = str(reference.get("display", variable))
            if variable not in converted_variables:
                missing.append(f"{display} (calculation is not converted)")
                continue
            source = _resolve_calculated_dependency_cell(
                plan, reference, nonrepeat_rows, repeating_rows)
            if source is None:
                missing.append(
                    f"{display} (unresolved or ambiguous target cell)")
            elif source in inactive_cells:
                # Definitively inactive targets are REDCap blanks, even if
                # the imported export contains a stale saved calculation.
                continue
            elif source in plan_cells or source in permanently_unavailable:
                resolved.append(source)
            else:
                missing.append(
                    f"{display} (target cell is not participating)")
        dependencies[cell] = tuple(dict.fromkeys(resolved))
        unresolved[cell] = tuple(dict.fromkeys(missing))

    previous_failures: dict[tuple[int, str], CalculationOutcome] = {}
    last_outcomes: dict[tuple[int, str], CalculationOutcome] = {}
    last_transition: set[tuple[int, str]] = set()
    converged = False
    nonconvergent: set[tuple[int, str]] = set()
    passes_run = 0
    max_passes = max(2, len({plan.variable for plan in plans}) + 2)

    for pass_number in range(1, max_passes + 1):
        subject_data, current_instances = _build_iteration_snapshot(
            rows, raw_states)
        current_values = {
            plan.row_index: _iteration_current_values(
                plan.row_index, rows, raw_states, nonrepeat_rows,
                fields_by_form)
            for plan in plans
        }
        outcomes: dict[tuple[int, str], CalculationOutcome] = {}
        updates: list[tuple[tuple[int, str], Any]] = []
        transitioned: set[tuple[int, str]] = set()

        for plan in plans:
            cell = (plan.row_index, plan.variable)
            blockers: list[str] = list(unresolved[cell])
            for dependency in dependencies[cell]:
                if dependency in permanently_unavailable:
                    blockers.append(
                        f"{dependency[1]} ({permanently_unavailable[dependency]})")
                elif dependency in previous_failures:
                    blockers.append(
                        f"{dependency[1]} ({previous_failures[dependency].status})")
            if blockers:
                outcome = CalculationOutcome(
                    status="blocked_dependency",
                    error="Multipass recalculation blocked by: "
                    + ", ".join(sorted(dict.fromkeys(blockers))),
                )
            else:
                subject_key = (plan.row.network, plan.row.subjectid)
                event_data: Mapping[Any, Any] = ChainMap(
                    current_instances.get((
                        plan.row.network, plan.row.subjectid,
                        plan.row.event_name), {}),
                    subject_data.get(subject_key, {}),
                )
                result = evaluate_converted_calculation(
                    str(plan.entry["converted_calculation"]),
                    current_values[plan.row_index],
                    event_data,
                ...
                if result.status != "ok":
                    ...
                    normalized_result = normalize_calculation_input(result.value)
                    outcome = CalculationOutcome(
                        status="ok", value=normalized_result)
                    previous_value = raw_states[plan.row_index].get(
                        plan.variable, "")
                    if (_multipass_state_key(previous_value)
                            != _multipass_state_key(normalized_result)):
                        updates.append((cell, normalized_result))
                        transitioned.add(cell)
            outcomes[cell] = outcome
            prior_failure = previous_failures.get(cell)
            if (
                (prior_failure is None) != (outcome.status == "ok")
                or (
                    prior_failure is not None
                    and (
                        prior_failure.status != outcome.status
                        or prior_failure.error != outcome.error
                    )
                )
            ):
                transitioned.add(cell)

        for (row_index, variable), value in updates:
            raw_states[row_index][variable] = value
        failures = {
            cell: outcome for cell, outcome in outcomes.items()
            if outcome.status != "ok"
        }
        passes_run = pass_number
        last_outcomes = outcomes
        last_transition = transitioned
        if not transitioned:
            converged = True
        ...
        nonconvergent.update(last_transition)

    if not converged:
        if not nonconvergent:
            nonconvergent.update(plan_cells)
        nonconvergent = _multipass_unstable_closure(
            nonconvergent, dependencies)
        variables = sorted({variable for _, variable in nonconvergent})
        artifacts.counters["multipass_nonconvergent_targets"] = len(
            nonconvergent)
        networks = tuple(sorted({plan.row.network for plan in plans}))
        artifacts.add_diagnostic(
                ("multipass_nonconvergence", *networks), {
            "type": "multipass_nonconvergence",
            "networks": list(networks),
            "passes_run": passes_run,
            "target_cell_count": len(nonconvergent),
            "variables": variables,
        })
    artifacts.counters["multipass_passes"] = passes_run
    artifacts.counters["multipass_converged"] = int(converged)
    ...
    def flush_row() -> None:
        ...
        _emit_result_rows(
            row_results, artifacts=artifacts,
            mismatch_sink=mismatch_sink)
        row_results.clear()

    for plan in sorted(plans, key=lambda item: (
            item.row_index, item.variable)):
        if current_row_index is not None and plan.row_index != current_row_index:
            flush_row()
        current_row_index = plan.row_index
        cell = (plan.row_index, plan.variable)
        common = {
            "structural_blanks": plan.structural_blanks,
            "comparison_mode": "multipass",
            "branching_eligibility": plan.branching_eligibility,
            "branching_eligibility_reason": plan.branching_reason,
            "extra_semantic_risks": plan.extra_semantic_risks,
        }
        if cell in nonconvergent:
            artifacts.counters["nonconvergent"] += 1
            row_results.append(_result_row(
                plan.row, plan.variable, plan.entry, recalculated="",
                status="nonconvergent",
                error=("Project-wide multipass recalculation did not "
                       f"converge after {passes_run} passes"),
                **common,
            ))
            continue
        outcome = last_outcomes[cell]
        if outcome.status != "ok":
            artifacts.counters[
                "blocked_dependencies" if outcome.status == "blocked_dependency"
                else "evaluation_errors"] += 1
            row_results.append(_result_row(
                plan.row, plan.variable, plan.entry, recalculated="",
                status=outcome.status, error=outcome.error, **common,
            ))
            continue
        artifacts.counters["evaluated"] += 1
        stored = normalize_calculation_input(
            plan.row.values.get(plan.variable, ""))
        if values_match(
        ...
        row_results.append(_result_row(
            plan.row, plan.variable, plan.entry,
            recalculated=outcome.value, status="mismatch", **common,
        ))
...
def scan_calculated_fields(
    ...
    """
    if comparison_mode not in {"direct", "propagated", "multipass"}:
    ...
    multipass_plans: list[_PropagationPlan] = []
    multipass_unavailable: dict[tuple[int, str], str] = {}
    multipass_inactive: set[tuple[int, str]] = set()
    ...
    for row_index, row in enumerate(loaded.rows):
        ...
        base_values: Mapping[str, Any] = (
            _calculation_input_values(base_row.values)
            if row.is_repeating and base_row is not None else {})
        current_row_values = _calculation_input_values(row.values)
        if row.is_repeating:
            ...
            imported_repeat_values.update(current_row_values)
            imported_current = imported_repeat_values
        else:
            imported_current = current_row_values
        ...
        for variable in available_targets:
            ...
            if (*row.identity, variable) in loaded.quarantined_values:
                ...
                if comparison_mode == "multipass":
                    multipass_unavailable[(row_index, variable)] = (
                        "conflicting imported target values")
                continue
            ...
            if inactive_reason:
                ...
                if comparison_mode == "multipass":
                    multipass_inactive.add((row_index, variable))
                continue
            ...
            if comparison_mode == "multipass":
                multipass_plans.append(_PropagationPlan(
                    row_index=row_index,
                    row=row,
                    variable=variable,
                    entry=entry,
                    structural_blanks=tuple(structural_blanks),
                    branching_eligibility=branching_eligibility,
                    branching_reason=branching_reason,
                    extra_semantic_risks=tuple(extra_risks),
                ))
                continue
            ...
            normalized_result = normalize_calculation_input(result.value)
            if comparison_mode == "propagated":
                propagated_current[variable] = normalized_result
            artifacts.counters["evaluated"] += 1
            stored = imported_current.get(variable, "")
            if values_match(
                    stored, normalized_result,
                    numeric_tolerance=numeric_tolerance):
                artifacts.counters["matches"] += 1
                continue
            ...
            artifacts.counters["mismatches"] += 1
            row_results.append(_result_row(
                row, variable, entry, recalculated=normalized_result,
                status="mismatch",
                structural_blanks=structural_blanks,
                comparison_mode=comparison_mode,
                branching_eligibility=branching_eligibility,
    ...
    if comparison_mode == "multipass":
        _scan_multipass_calculated_fields(
            loaded,
            multipass_plans,
            calculations,
            fields_by_form,
            multipass_unavailable,
            inactive_cells=multipass_inactive,
            artifacts=artifacts,
            numeric_tolerance=numeric_tolerance,
            blank_result_as_zero=blank_result_as_zero,
            mismatch_sink=mismatch_sink,
...
def merge_scan_metadata(parts: Sequence[ScanArtifacts]) -> ScanArtifacts:
    ...
    special = {"multipass_converged", "multipass_passes"}
    multipass_parts = [
        part for part in parts
        if "multipass_converged" in part.counters
    ]
    for part_index, part in enumerate(parts):
        merged.counters.update({
            key: value for key, value in part.counters.items()
            if key not in special
        })
        for key, diagnostic in part.structural_diagnostics.items():
            unique_key = key
            if unique_key in merged.structural_diagnostics:
                unique_key = (*key, "partition", part_index)
            merged.add_diagnostic(unique_key, diagnostic)
    if multipass_parts:
        merged.counters["multipass_converged"] = int(all(
            part.counters.get("multipass_converged", 0)
            for part in multipass_parts
        ))
        merged.counters["multipass_passes"] = max(
            part.counters.get("multipass_passes", 0)
            for part in multipass_parts
        )
    return merged
...
def build_diagnostics(
    ...
    result: dict[str, Any] = {
        ...
        "branching_logic": {
            ...
            "unknown_result_actionability": "suppressed_from_output",
        },
        "comparison_mode": comparison_mode,
        "cross_event_mode": CROSS_EVENT_MODES.get(
            comparison_mode, "unknown"),
        ...
        "mismatch_output_mode": (
            "deterministic_visible_nonblank_row_streaming"),
        "inputs": input_details,
        "summary": {
            "input_file_count": len(input_details),
            "input_row_count": sum(
                int(item["row_count"]) for item in input_details),
            "missing_code_cells_replaced_with_blank": sum(
                int(item.get(
        ...
        "runtime_notes": [
            (
                "Imported REDCap missing codes are presented as blank during "
                "CSV iteration, stored-value comparison, branching checks, "
                "and every calculation field lookup. Formula literals retain "
                "their expression semantics, but the final calculated root "
                "result is normalized before comparison and propagation."
            ),
            (
                "Stored-value comparison alone treats actual blanks, "
                "whitespace-only text, and trimmed case-insensitive literal "
                "NaN as one blank class; calculation inputs retain their text."
            ),
            (
                "Output contains only rows whose branching eligibility is "
                "visible and whose stored or recalculated value is outside the "
                "comparison blank class. Evaluation counters include suppressed "
                "rows; output_suppressed__ counters record the exclusions."
            ),
            (
            ...
            (
                ...
                "Legacy propagated mode updates same-event calculated "
                "dependencies in translator order; explicit cross-event "
                "values remain an immutable stored snapshot."
                if comparison_mode == "propagated" else
                "Multipass mode evaluates all eligible targets synchronously "
                "from the prior complete project state, including explicit "
                "cross-event calculated references, until exact convergence. "
                "Evaluation-blocked dependencies and nonconvergence are "
                "reported as not evaluable."
            ),
            (
                "Multipass dependency blocking is deliberately conservative: "
                "all statically translated calculated references are checked, "
                "including references in lazy branches that may not execute. "
                "Definitively inactive calculated targets are instead exposed "
                "as blank state."
            ),
            (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--comparison-mode",
        choices=("direct", "propagated", "multipass"),
        default="multipass",
        help=(
            "direct uses the imported snapshot for every formula; propagated "
            "keeps legacy ordered same-event updates; multipass synchronously "
            "recalculates same- and cross-event dependencies to convergence. "
            "Multipass nonconvergence writes artifacts, then returns exit "
            "status 1 (default: %(default)s)."))
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        args.comparison_mode == "multipass"
        and artifacts.counters.get("multipass_nonconvergent_targets", 0)
    ):
        return 1
