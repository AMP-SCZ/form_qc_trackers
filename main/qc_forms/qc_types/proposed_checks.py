...
class ProposedChecks(FormCheck):
    ...
    @classmethod
    def _render_evidence_value(cls, value: Any) -> str:
        """Render one exact value without creating tracker delimiters.

        Safe scalars stay human-readable. Values whose text would be mutated by
        the tracker transport use a tagged JSON string with pipe/semicolon
        characters represented as Unicode escapes. Long values retain the old
        readable preview but end in ``...`` so Analyze Flags fails closed.
        """

        text = str(value)
        sanitized = cls._sanitize_evidence_text(text)
        needs_tagged_encoding = (
            sanitized != text
            or text.casefold() in {
                "<blank>", "<unavailable>", "[redacted]"}
            or text.startswith("@json")
            or text.endswith("...")
        )
        if needs_tagged_encoding:
            encoded = json.dumps(text, ensure_ascii=True)
            # These two delimiters are consumed before evidence parsing, so
            # keep them out of the rendered tracker cell even inside quotes.
            encoded = encoded.replace("|", r"\u007c").replace(";", r"\u003b")
            rendered = "@json:" + encoded
            if len(rendered) <= MAX_EVIDENCE_VALUE_CHARS:
                return rendered
            preview = cls._sanitize_evidence_text(text)
            if not preview.endswith("..."):
                preview = preview[:MAX_EVIDENCE_VALUE_CHARS - 3].rstrip() + "..."
            return preview
        return cls._sanitize_evidence_text(text)
            ...
            if variable not in self._raw:
                ...
                if not is_null:
                    try:
                        ...
                        is_null = False
                # Textual missing sentinels such as "NA" are real stored
                # values. Only a true null or an actual empty string is encoded
                # as the exact blank state.
                if is_null or (isinstance(value, str) and value == ""):
                    rendered = "<blank>"
                else:
                    rendered = self._render_evidence_value(value)
    ...
    def _parsed_series(self, variable: str) -> pd.Series:
        ...
        def parse_local_datetime(value):
            # Parse each value independently: mixed offsets/formats must not
            # affect neighboring rows. QC compares recorded local dates/times,
            # so discard timezone metadata without shifting the clock to UTC.
            parsed = pd.to_datetime(value, errors="coerce")
            if pd.isna(parsed):
                return pd.NaT
            return parsed.tz_localize(None)

        return pd.to_datetime(cleaned.map(parse_local_datetime), errors="coerce")
