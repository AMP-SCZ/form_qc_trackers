...
class GeneralChecks(FormCheck):  
    ...
    def check_if_blank(self, row, filtered_forms,
        ...
        """
        # This field is intentionally excluded only from blank checks, including
        # runs using an older general_check_vars.json that still lists it.
        if all_vars and all_vars[0] == 'chrpharm_interview_date':
            return
    ...
    def check_form_completion(self,row):
        ...
        if cohort.lower() not in ["hc", "chr"]:
            return
        curr_tp_forms = self.forms_per_tp[cohort.lower()][self.timepoint]
    ...
    def guid_format_check(self, row, filtered_forms,
        ...
        if not re.search(r"^NDAR[A-Z0-9]+$", guid):
            rendered_guid = escape_specific_flag_value(guid)
            error_message = (
    ...
    def missing_data_form_consistency_check(self, row):
        ...
        if has_domain_trigger and not has_whole_tp_trigger:
            ...
            for code, domain_info in selected_domains:
                ...
                if not domain_forms:
                    # Nothing from this domain is expected at this visit, so
                    # the selection has no actionable form-level contradiction.
                    # Do not flag the operator merely because the domain map
                    # and this cohort/timepoint schedule have an empty
                    # intersection.
                    continue
