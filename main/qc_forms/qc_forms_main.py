...
class QCFormsMain():
    def __init__(self):
        ...
        # Scheduled-form lookups disagree on cohort-key casing across the
        # codebase: every per-row checker keys this dependency by
        # `cohort.lower()` (form_check.standard_qc_check_filter,
        # general_checks.check_form_completion,
        # clinical_checks_main.call_twenty_one_day_check), while other
        # consumers key it by `cohort.upper()`
        # (analyze_dataset/collect_misc_data.py). A file written with either
        # casing crashes half its consumers mid-run with a bare KeyError on
        # row N. Expose both casings, once, at load, on this run's copy only
        # -- the cached dependency and every other consumer are untouched, and
        # the alias points at the same schedule object, so cohort casing can
        # never decide whether a check runs. Timepoint keys are deliberately
        # left alone; only the cohort level is aliased.
        schedule = self.form_check_info['forms_per_timepoint']
        aliased_schedule = dict(schedule)
        for cohort_key, cohort_schedule in schedule.items():
            stripped = str(cohort_key).strip()
            for alias in (stripped.lower(), stripped.upper()):
                aliased_schedule.setdefault(alias, cohort_schedule)
        self.form_check_info['forms_per_timepoint'] = aliased_schedule
        # Fail here rather than partway through a network/timepoint. Both
        # cohorts are required by the per-row checks, so an absent schedule
        # is a broken dependency file, not a condition to work around.
        missing_cohorts = [cohort for cohort in ('chr', 'hc')
                           if cohort not in aliased_schedule]
        if missing_cohorts:
            raise KeyError(
                f"forms_per_timepoint.json is missing cohort schedule(s) "
                f"{missing_cohorts}; found top-level key(s) "
                f"{sorted(str(key) for key in schedule)}. Every per-row "
                f"scheduled-form check keys this file by cohort, so the run "
                f"would otherwise fail partway through with a bare KeyError.")
