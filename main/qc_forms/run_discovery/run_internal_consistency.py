"""
Operator entry point for internal-consistency checks
...
def main():
    if _CONFIG_PATH is None:
        print(
            "[run_internal_consistency] FATAL: config.json not "
    ...
    if not enabled:
        print(
            "[run_internal_consistency] discovery."
            "internal_consistency disabled in config.json (default)."
        )
        return 0
    InternalConsistencyChecks(config_dict=cfg).run()
    return 0
