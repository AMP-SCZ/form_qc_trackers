"""
Operator entry point for the per-site scale-relationship correlation
drift detector (Layer 2 / discovery, default-off).

Reads config.json from the project root. Reads the curated pair
list from dependencies/expected_scale_relationships.json (or from
config.discovery.site_correlation_drift.pairs if specified).
...
from qc_types.discovery.site_correlation_drift_checks import (  # noqa: E402
...
def main():
    if _CONFIG_PATH is None:
        print(
            "[run_site_correlation_drift] FATAL: config.json not "
    ...
    if not enabled:
        print(
            "[run_site_correlation_drift] "
            "discovery.site_correlation_drift disabled in "
            "config.json (default). Set "
            "discovery.site_correlation_drift.enabled=true to run.")
        return 0

    detector = SiteCorrelationDriftChecks(config_dict=cfg)
    detector.run()
    return 0
