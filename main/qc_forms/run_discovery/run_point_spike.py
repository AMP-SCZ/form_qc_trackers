"""
Operator entry point for point-spike detection
...
Discovery-tier semantics:
  ...
  - writes only combined_outputs/discovery/
    point_spike_candidates.parquet
...
def main():
    if _CONFIG_PATH is None:
        print(
            "[run_point_spike] FATAL: config.json not found in any "
            f"ancestor of {_HERE}"
        )
        return 1
    ...
    if not enabled:
        print(
            "[run_point_spike] discovery.point_spike disabled in "
            "config.json (default). Set discovery.point_spike."
            "enabled=true to run."
        )
        return 0

    detector = PointSpikeChecks(config_dict=cfg)
    detector.run()
    return 0
