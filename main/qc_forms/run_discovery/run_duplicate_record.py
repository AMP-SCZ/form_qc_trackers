"""
Operator entry point for duplicate-record candidate detection
...
Discovery-tier semantics:
  - reads only the per-tp combined REDCap CSVs (does not modify
    any source file)
  ...
  - near-duplicate detection requires sklearn at runtime; exact
    duplicates run regardless. See module docstring for details.
"""
...
def main():
    if _CONFIG_PATH is None:
        print(
            "[run_duplicate_record] FATAL: config.json not found in any "
            f"ancestor of {_HERE}"
        )
        return 1
    ...
    if not enabled:
        print(
            "[run_duplicate_record] discovery.duplicate_record "
            "disabled in config.json (default). Set "
            "discovery.duplicate_record.enabled=true to run."
        )
        return 0

    detector = DuplicateRecordChecks(config_dict=cfg)
    detector.run()
    return 0
