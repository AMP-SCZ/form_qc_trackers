"""
Operator entry point for date anomaly detection
...
Discovery-tier semantics:
  - reads only the dates long-format parquet (does not modify
    any source file)
  - writes only combined_outputs/discovery/
    date_anomaly_candidates.parquet
...
def main():
    if _CONFIG_PATH is None:
        print(
            "[run_date_anomaly] FATAL: config.json not found in any "
            f"ancestor of {_HERE}"
        )
        return 1
    ...
    if not enabled:
        print(
            "[run_date_anomaly] discovery.date_anomaly disabled "
            "in config.json (default). Set "
            "discovery.date_anomaly.enabled=true to run."
        )
        return 0

    detector = DateAnomalyChecks(config_dict=cfg)
    detector.run()
    return 0
