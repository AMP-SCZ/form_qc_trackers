...
def detect_all(per_slice: Dict, classify: Dict = None, **_) -> List[dict]:
    ...
    # Collapse the per-site repetition: when one variable trips the SAME drift
    # sub-test at many sites, emit one row listing the sites instead of one row
    # per site (see _collapse_multisite).
    return _collapse_multisite(rows)


def _collapse_multisite(rows: List[dict]) -> List[dict]:
    """Fold multi-site duplication into one row per (network, variable, method).

    site_network emits one row per (site, variable, drift sub-test). When the
    SAME variable trips the SAME sub-test at several sites, that is one finding
    ("this variable drifts across the network"), not N. Group by
    (network, variable, method) -- so the distinct drift KINDS (median / spread
    / missingness / correlation) stay as separate rows, but the per-site
    repetition within a kind folds into a single row that names the affected
    sites, keeps the strongest site's severity, and lists each site's observed
    value. The concrete sites are preserved in a ``sites`` column, and each
    site's OWN drift severity in a parallel ``site_severities`` column (same
    order), so the runner's cross-detector outlier suppression keys on the
    concrete site AND caps by that site's own severity rather than the group max
    -- see SimpleAnomalyDetector._suppress_redundant_outliers. ``sites``,
    ``site_severities`` and the per-site ``observed_value`` list all share one
    severity-descending order so the row's listings never disagree.

    Cross-network rows (site_id == "(network-level)") are per-network already,
    not multi-site, so they pass through unchanged. Single-site groups pass
    through too, gaining only the ``n_sites``/``sites``/``site_severities``
    columns for uniformity.
    """
    if not rows:
        return rows
    ...
    for r in rows:
        ...
        # Concrete per-site rows have a 2-char site code; network-level and any
        # other non-site rows (site_id blank or parenthesised) are left alone.
    ...
    def _sev(g) -> float:
        try:
            return float(g.get("severity_score", 0) or 0)
    ...
    for (network, variable, method), grp in groups.items():
        ...
        by_sev = sorted(grp, key=_sev, reverse=True)
        ordered_sites = [str(g.get("site_id", "")) for g in by_sev
                         if str(g.get("site_id", ""))]
        sev_str = ", ".join(f"{_sev(g):g}" for g in by_sev)
        rep = dict(by_sev[0])               # strongest site is the representative
        rep["n_sites"] = len(ordered_sites)
        rep["sites"] = ", ".join(ordered_sites)
        rep["site_severities"] = sev_str
        if len(ordered_sites) <= 1:
            out.append(rep)
            continue
        top_site = ordered_sites[0]
        bits = [f"{g.get('site_id', '')}: {g.get('observed_value', '')}"
                for g in by_sev[:SHOWN]]
        listed = "; ".join(bits)
        if len(by_sev) > SHOWN:
            listed += f"; (+{len(by_sev) - SHOWN} more)"
        rep["site_id"] = f"({len(ordered_sites)} sites)"
        rep["observed_value"] = listed
        rep["explanation"] = (
            f"{variable} shows this site-level shift at {len(ordered_sites)} sites "
            f"({', '.join(ordered_sites[:8])}{'...' if len(ordered_sites) > 8 else ''}); "
            f"strongest at site {top_site} (severity {rep.get('severity_score')}). "
            f"Collapsed from {len(grp)} per-site rows; per-site values in observed_value."
        )
        out.append(rep)
