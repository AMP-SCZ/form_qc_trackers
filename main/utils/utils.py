...
def _load_config(absolute_path):
    ...
    if cached is None:
        config_path = f'/home/ob001/formqc_new_copy_6_24/form_qc_trackers/config.json'
...
class Utils():
    def __init__(self):
        ...
        # Operational runs can be scoped to one network without changing the
        # long-standing default. The environment variable is intentionally
        # ephemeral so a one-off PRESCIENT run cannot silently change future
        # scheduled runs. Every pipeline stage consumes this single validated
        # list, including Dropbox readback/upload.
        configured_networks = os.environ.get('QC_NETWORKS')
        if configured_networks is None:
            configured_networks = self.config_info.get(
                'pipeline_networks', ['PRONET'])
        ...
        if (not isinstance(configured_networks, (list, tuple))
            ...
            raise RuntimeError(
                "QC network scope must be a non-empty list or a comma-"
                "separated QC_NETWORKS value.")
        normalized_networks = []
