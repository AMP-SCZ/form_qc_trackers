...
RULES = [
    ...
    '((df["chrbprs_interview_date"] - df["chrcdss_interview_date"]).dt.days == 0) & (df["chrcdss_calg8"] == 0) & (df["chrbprs_bprs_suic"] > 3)',
    '((df["chrbprs_interview_date"] - df["chrcdss_interview_date"]).dt.days == 0) & (df["chrcdss_calg8"] > 1) & (df["chrbprs_bprs_suic"] != -9) & (df["chrbprs_bprs_suic"] <= 2)',
    ...
    '((df["chrassist_interview_date"] - df["chrscid_interview_date"]).dt.days.abs() < 1) & (df["chrassist_whoassist_use4"] == 1) & (df["chrscid_stimulant_yn"] == 0)',
...
RULE_LABELS = {
    ...
    98: "ASSIST lifetime cocaine use = Yes, but SCID stimulant use = No",
...
ENABLED_RULE_NUMBERS = frozenset({
    ...
    39,
    86, 88,
    94, 96, 98, 100, 101, 102, 104,
    122,
    141, 144,
})

# Retain this source-catalog metadata hook for diagnostics and future rules
# whose evidence can be supplied by interchangeable fields.  The operational
# Cross Checks tab intentionally applies no separate observation/missing-code
# mask; each rule expression alone decides whether a row is a hit.
...
DISABLED_RULE_REASONS = {
    ...
    29: "Historical CHS dictionaries asked whether the participant was currently menstruating, not whether menarche had occurred.",
    32: "Historical CHS dictionaries asked whether the participant was currently menstruating, not whether menarche had occurred.",
    ...
    106: "ASSIST and SCID 'Other' are instrument-relative categories and cannot be compared without a specified substance mapping.",
    ...
    126: "CHS records last use before the blood draw, while ASSIST covers the past month through its interview; same-day ordering is unavailable.",
    127: "CHS records last use before the blood draw, while ASSIST covers the past month through its interview; same-day ordering is unavailable.",
    ...
    134: "CHS records last use before the blood draw, while saliva asks about any use today; same-day ordering is unavailable.",
    135: "CHS records last use before the blood draw, while saliva asks about any use today; same-day ordering is unavailable.",
...
def _parse_calendar_dates(raw: pd.Series) -> pd.Series:
    """Parse dates independently of neighboring values and pandas version.

    Pandas 2 may infer one format for a complete Series, which makes a valid
    ISO datetime become ``NaT`` when the first nonblank value is date-only
    (and vice versa).  REDCap's ISO forms all share an unambiguous date
    prefix, so parse that prefix explicitly and use scalar parsing only for
    non-ISO legacy values.  The scalar fallback intentionally prevents one
    row's representation from determining how another row is interpreted.
    """
    text = raw.astype("string").str.strip()
    # pandas 1.4's default mask replacement is incompatible with StringArray.
    # Supply its native missing value explicitly before converting to dates.
    text = text.mask(
        text.isin(DATE_MISSING_CODES)
        | text.str[:10].isin(DATE_MISSING_CODES),
        other=pd.NA,
    )

    iso_date = text.str.extract(
        r"^(\d{4}-\d{2}-\d{2})(?:$|[T\s])", expand=False)
    parsed = pd.to_datetime(
        iso_date, format="%Y-%m-%d", errors="coerce")

    legacy_mask = text.notna() & iso_date.isna()
    if legacy_mask.any():
        ...
        parsed.loc[legacy_mask] = text.loc[legacy_mask].map(parse_scalar)
...
def extract_filter_columns(
    rule_text: str,
    available_columns: list[str] | tuple[str, ...] | pd.Index,
    *,
    rule_number: int | None = None,
) -> list[str]:
    """Expand trusted ``df.filter(regex=...)`` operands in source order.

    The original pharmaceutical prototypes select repeating medication slots
    with dataframe filters instead of direct ``df["field"]`` references.  A
    direct-reference-only extractor silently discarded those operands before
    evaluating the rule.
    """
    patterns = list(dict.fromkeys(re.findall(
        r'df\.filter\(regex=r"([^"]+)"\)', rule_text)))
    matched = []
    for pattern in patterns:
        compiled = re.compile(pattern)
        pattern_matches = [
            column for column in available_columns
            if compiled.search(str(column))]
        if not pattern_matches:
            context = (
                f"Rule {rule_number}" if rule_number is not None
                else "Rule expression"
            )
            raise KeyError(
                f'{context} matrix operand df.filter(regex=r"{pattern}") '
                "matched zero columns; refusing to evaluate it as an "
                "all-False matrix")
        matched.extend(pattern_matches)
    return list(dict.fromkeys(matched))


_PHARM_CODE_COLUMN = re.compile(
    r"^chrpharm_med\d+_(?:tp|name|name_past)$")


def _normalize_pharm_code_strings(raw: pd.Series) -> pd.Series:
    """Canonicalize numeric-equivalent medication codes as strings.

    CSV type inference commonly turns ``238`` into ``238.0`` when a repeating
    medication column also contains blanks.  The legacy matrix expressions
    compare string codes, so normalize integral numeric representations while
    preserving nonnumeric timepoint tokens.
    """
    text = raw.astype("string").str.strip()
    ...
    if integral.any():
        text.loc[integral] = (
            numeric.loc[integral].astype("Int64").astype("string"))
    return text
...
def prepare_dataframe(
    ...
    for column in selected:
        if column in DATE_COLUMNS:
            out[column] = _parse_calendar_dates(out[column])
        elif _PHARM_CODE_COLUMN.fullmatch(column):
            out[column] = _normalize_pharm_code_strings(out[column])
        else:
...
def prep_df_for_rule(df: pd.DataFrame, rule_text: str) -> pd.DataFrame:
    """Prepare direct and matrix-selected inputs for standalone diagnostics."""
    columns = extract_columns(rule_text)
    columns.extend(extract_filter_columns(rule_text, df.columns))
...
def evaluate_rule_mask(
    ...
    # ``DataFrame.filter`` returns an empty dataframe when a regex selects no
    # fields, and ``.any(axis=1)`` then silently becomes False for every row.
    # Validate each matrix operand again here because callers may supply a
    # manually prepared dataframe instead of using ``prep_df_for_rule``.
    extract_filter_columns(
        rule_text, prepared_df.columns, rule_number=rule_number)
    missing = [
